...
async def upload_files(file: UploadFile = File(...)):
    ...
    cv2.imwrite(filename=output_filename, img=annotated)
...
async def detect_video(file: UploadFile):
    ...
    annotated_video = cv2.VideoWriter(
        ...
        fps=output_video.get(cv2.CAP_PROP_FPS),
        frameSize=(
            int(output_video.get(cv2.CAP_PROP_FRAME_WIDTH)),
            int(output_video.get(cv2.CAP_PROP_FRAME_HEIGHT)),
        ),
    )
